...
#***************************************************
    ...
    */
    public function GetPageInfo($id = 0, $pagename = '', $active = 0)
    {
        ...
        {
            global $gSmarty;
            throw new Exception($gSmarty -> _config[0]['vars']['errnfound']);
        }
        ...
        $row['link']     = $link.'pctg='.$row['cid'].'&pn='.$row['pagename'];
        return $row;
    ...
    */
    public function GetPageList($ctg = 0, $active = 0, $sort = '', $nuse = 0)
    {
        ...
        $sql = 'SELECT id, pagename, name, active, sortid FROM '.$this -> mTable.
               ' WHERE cid = "'.$ctg.'"';
        if ($active == 1)
            $sql .= ' AND active = 1';
        if ($nuse > 0)
        ...
        {
            $row['name']     = stripslashes($row['name']);
            $row['link']     = $link.'pctg='.$ctg.'&pn='.$row['pagename'];
            $res[]           = $row;
        }
        return $res;
    ...
    */
    public function GetCatInfo($cid = 0)
    {
        ...
        {
            global $gSmarty;
            throw new Exception($gSmarty -> _config[0]['vars']['errnfound'].' [cid = '.$cid.']');
        }
        $link = $this -> mLink;
        if ( strpos($link, '?') > 0 )
            $link .= '&';
