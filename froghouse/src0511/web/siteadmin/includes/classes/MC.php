...
class MC
{
     ...
     * @return void
     */
    public function __construct($dbPtr, $table = 'mc', $tableUsers = 'users', $tags = '', &$uid, &$geo, $resOnPage = 20)
    {
        $this -> mDbPtr      =  $dbPtr;
        $this -> mTable      =  $table;
     ...
     */
    public function Add($toid, $fromid, $subject, $message, $sendtime = 0, $type = 0)
    {
        $toid    = intval($toid);
        $fromid  = intval($fromid);
        $subject = text_filter($subject, '', true);
        $message = text_filter($message, $this -> mAllowTags, true);
        $type    = intval($type);

        if (0 == $sendtime)
           $sendtime = date('Y-m-d H:i:s', m_time());

        $vals = array($subject, $message);

        $this->mDbPtr->query('INSERT INTO '.$this -> mTable.'
                              (toid, fromid, sendtime, subject, message,type) 
                              VALUES ('.$toid.',
                                      '.$fromid.',
                                      \''.$sendtime.'\',
                                       ?,?,'.$type.')', $vals);
    ...
    public function GetLastKiss($uid)
    {
        $uid = intval($uid);
        return $this->mDbPtr->getRow('SELECT id, subject
                                      FROM '.$this->mTable.'
                                      WHERE toid='.$uid.' 
                                            AND deleted_by_to=0
                                            AND type=1
    ...
    public function &GetAll($uid, $type = 0, $pStart = -1, $order = 0)
    {
        ...
        {
            case 1:   // by from ASC
                $OrderingStr = ' ORDER BY mu.nickname ASC, 
                                          mc.sendtime DESC';
                break;
            case 2:   // by from DESC
                $OrderingStr = ' ORDER BY mu.nickname DESC, 
                                          mc.sendtime DESC';
                break;
            case 3:   // by date ASC
        ...
        $sql = 'SELECT mc.id, mc.toid, mc.fromid, mc.sendtime,
    ...
    public function GetNextPrevMessage($id, $uid, $type = 0, $order = 0)
    {
        $uid = intval($uid);
        switch ($order)
        {
        ...
        $sql = 'SELECT mc.id, mc.sendtime, mc.subject 
                FROM '.$this -> mTable.' mc, '.$this ->mTableUsers.' mu
                WHERE '.$clause.
                $OrderingStr;
