...
class ThumbImage
{
                      ...
                      )
    {
        if ($ImageDir == '')
            $ImageDir = DIR_WS_IMAGE .'/';
        if ($CacheDir == '')
            $CacheDir = DIR_WS_IMAGE . '/' . DIR_NAME_IMAGECACHE2;
        if ($ImageDirRel == '')
            $ImageDirRel = PATH_ROOT . DIR_NAME_IMAGE . '/';
        if ($CacheDirRel == '')
            $CacheDirRel = PATH_ROOT . DIR_NAME_IMAGE . '/' . DIR_NAME_IMAGECACHE2;
    ...
    */
    public function CreateThumbImage($x, $y, $aspectratio = 1)
    {
        ...
        if ($imagedata[0] < $x && $imagedata[1] < $y && !$iscached)
        {
            $x = $imagedata[0];
            $y = $imagedata[1];
            $makethumb = true;
