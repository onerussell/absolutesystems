...
echo "<table cellpadding='0' cellspacing='1' bgcolor='#DDDDDD'>";
...
if ($i<10) $i='0'.$i;
   ...
   while(($file = $mydir->read()) !== false) {
      ...
      if( ($filename[1] == 'png') OR ($filename[1] == 'jpg')) {
    	...
    	if(!file_exists("$projectpath/img/".$filename[0]."_x".$i.".".$filename[1])) {

        	cropImageJPG("$path/$file","$projectpath/img/".$filename[0]."_x".$i.".".$filename[1],200,150,95);

        }

      	echo '<td><a href="x'.$i.'/" name="x'.$i.'"><img src="img/'.$filename[0].'_x'.$i.'.'.$filename[1].'" border="10" style="border-color:#FFFFFF" width="200" height="150" border="0"/></a></td>';
      	$k++;
      	if ($k>3) {echo '</tr>'; $k=0;}
      }
   }
